...
3. **레퍼런스 앵커 주입** — 등장 캐릭터마다 `04_visual/refs/{IDTAG}_*.png` 레퍼런스 시트를 외형 기준으로 참조하도록 프롬프트에 명시한다(예: "match the exact appearance of {IDTAG} as defined in the locked character reference sheet: <불변 토큰>"). 토큰만이 아니라 확정 레퍼런스가 일관성의 닻이다. 앵커 **경로**는 character-sheets.md 확정 시점에 결정적(`refs/{IDTAG}_ref.png`)이므로 레퍼런스 렌더 완료를 기다리지 않고 프롬프트를 작성한다(크리티컬 패스 병렬) — 단, 렌더된 레퍼런스가 외형 변경을 강제하면(ref-sheet-artist 통지, 드묾) 영향 패널 프롬프트를 **패널 렌더 전에** 패치한다.
...
- 입력:
  ...
  - `04_visual/refs/INDEX.md` — 캐릭터별 확정 레퍼런스 시트 경로(외형 앵커; 병렬 진행 중엔 character-sheets.md의 결정적 경로 사용)
  - `06_assembly/retro_ep{NN-1}.md` — **직전 회차 회고(있으면, 새 회차 프롬프트 작성 전 필수 Read)**: 축별 REGEN 집계와 효과가 검증된 보강 패턴(예: 레터링 잉크 통합 문구)을 처음부터 선적용해 같은 결함 클래스의 재발을 막는다.
- 출력:
...
- **panel-validator REGEN 수용(핵심)**: panel-validator가 특정 패널을 REGEN으로 되돌리면, 사유(C1~C6)에 맞춰 그 패널 프롬프트만 보강한다 — 예: 배경 급변→장소 토큰 강화, 한글 깨짐→텍스트 따옴표·굵게·짧게, 외형 이탈→레퍼런스 앵커·식별 표식 강조, 구도 어긋남→앵글 명시. **보강은 `ep{NN}_prompts.md`에 반영하는 것이 정본이다** — `_render_jobs/`의 job `.txt`만 직접 고치면 다음 재분할이 보강을 덮어쓴다(EP01 P32/P49 실사례). 보강 후 담당 panel-artist에 재렌더를 요청한다(루프 — 재렌더는 누적 ≤5장 웨이브로 디스패치됨).
- **카나리아 시스템성 결함 수용(전 패널 일괄 수정)**: panel-validator가 카나리아 5장에서 **같은 축 결함 ≥2장(시스템성)**을 보고하면, 개별 패널 보강이 아니라 **전 패널 프롬프트의 공통 패턴**을 정본 `ep{NN}_prompts.md`에서 일괄 수정하고(예: 레터링 표현을 잉크 통합 문구로 전면 교체) `python _split_prompts.py …` 재분할까지 마친 뒤 렌더 재개를 알린다. (EP01 오버레이-룩이 13/57장에 퍼진 사고의 재발 방지 — 카나리아 단계에 잡으면 수정 1회로 끝난다.)
...
- **단일 패널의 현재 프롬프트 확인은 `_render_jobs/panels/panel_NNN.txt`(소형)를 Read**한다 — 패널 1~2장 보강에 전체 `ep{NN}_prompts.md`(~60KB)를 Read하지 않는다(토큰). 보강 결과의 정본 반영 규칙은 위 "REGEN 수용" 그대로(md가 정본).
