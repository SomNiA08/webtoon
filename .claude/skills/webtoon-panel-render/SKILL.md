...
- 내부에서 5개 단위로 `wait`하므로 동시 세션이 5를 넘지 않는다.
...
3. 그 패널들만 5장 이하 배치로 재렌더.
4. **재렌더 원자 규약 완주(생략 금지)**: ① panel-validator 6축 **전체** 재검증 — 고치려던 결함만 확인하고 끝내지 않는다(재렌더는 새 결함을 만든다: EP01 P32 재렌더에서 하루 외형은 고쳐졌으나 도경 안경이 사라짐) → ② validation.md·qa_report.md 해당 항목 갱신 → ③ `ep{NN}_manifest.md5` 재생성 → ④ RELEASE에 이미 패키징된 회차면 해당 파일 재복사.
