...
## 핵심 역할
1. **그룹 C REGEN 재렌더 (생성-검증 루프)** — **panel-validator**가 REGEN으로 되돌린 그룹 C 패널을, prompt-smith가 보강한 프롬프트로 **그 패널만** 재렌더한다(말풍선·한글 대사 in-image 베이크 유지). validator가 ACCEPT/ACCEPT-FLAG할 때까지 반복(패널당 최대 3회).
