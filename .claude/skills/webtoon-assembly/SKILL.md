...
### 절차
1. `05_panels/ep{NN}/`의 PNG를 파일명 오름차순(panel_001 → …)으로 수집한다(panel-validator 통과본).
2. 템플릿의 `PANELS` 배열을 채운다. 한 항목 = 한 패널(말풍선 데이터 없음):
   ...
   - `w`/`h`는 원본 PNG 픽셀 치수 — 지정하면 이미지 로드 전에 칸 높이가 예약되어 **레이아웃 시프트(CLS) 없이** 스크롤 위치가 안정된다(110MB급 회차에서 특히 중요). `file 05_panels/ep{NN}/panel_*.png` 출력의 "W x H"를 그대로 옮긴다(추가 도구 불필요).
...
- **로딩 전략(50+장 × 장당 ~2MB ≈ 회차 110MB+)**: 선두 2~3장은 `loading="eager"`(첫 장 `fetchpriority="high"`)로 첫 화면을 즉시 띄우고, 나머지는 `loading="lazy" decoding="async"` + IntersectionObserver 프리페치(뷰포트 ~2400px 앞서 로드)로 **스크롤 도달 전에 로드를 끝낸다**. `w`/`h` 치수 지정으로 로드 전 칸 높이를 예약한다(CLS 방지). 전부 템플릿에 반영되어 있다 — 데이터만 채우면 된다.
...
### 객관 지표 측정 (스크립트로)
- 패널 수: `05_panels/ep{NN}/`의 `panel_*.png` 개수를 센다.
- 0바이트: 파일 크기 0인 PNG를 찾는다.
- 손상: PNG 매직 바이트(`\x89PNG`)로 헤더를 확인한다.
- **md5 중복**: `md5sum panel_*.png | awk '{print $1}' | sort | uniq -d` (macOS는 `md5 -r`) — 비어야 정상(서로 다른 패널이 같은 이미지면 사고). EP01에서 실제 발생했으므로 필수. (참조: `references/qa-checks.md`)
- **매니페스트 정합**: `(cd 05_panels/ep{NN} && md5sum -c ../../04_visual/ep{NN}_manifest.md5)` — 검증 시점 이후 파일이 바뀌지 않았는지 확인. 불일치 = 무검증 재렌더가 끼어든 것이므로 panel-validator 재검증으로 되돌린다. (일괄 검사는 `bash _verify_release.sh {NN}`)
...
**재렌더 원자 규약(EP01 실사고 — QA 이후에도 동일 적용)**: FIX/REDO로 패널이 1장이라도 재렌더되면 ① panel-validator 6축 **전체** 재검증(고치려던 결함만 보지 말 것 — 재렌더는 새 결함을 만든다) → ② validation.md·qa_report.md 해당 항목 갱신 → ③ `04_visual/ep{NN}_manifest.md5` 재생성 → ④ RELEASE에 이미 패키징됐다면 재복사까지 한 묶음으로 완주한다. EP01에서 이 규약 없이 QA 후 재렌더된 P32·P49가 무검증 릴리스되어 P49에 캐릭터 외형 위반이 유출됐다.
...
**md5 릴리스 게이트(사인오프 전 필수)**: `bash _verify_release.sh {NN}` — 패널 수·0바이트/손상·md5 중복·**검증 매니페스트 정합**·RELEASE 사본 무결성을 일괄 검사한다. exit 0이어야 패키징한다. 하나라도 불일치(FAIL)하면 검증받지 않은 바이트가 끼어든 것이므로 **사인오프를 거부**하고 재렌더 원자 규약(§3)으로 되돌린다. 패키징 후에도 1회 재실행해 RELEASE 사본 무결성([5])까지 PASS를 확인한다.

**패키징에서 재인코딩 금지**: RELEASE 사본은 검증본과 **바이트 동일**해야 한다(매니페스트 = 검증된 바이트). 용량 최적화(pngquant 등)가 필요하면 렌더 단계에서 **매니페스트 생성 전**에만 수행한다(`webtoon-panel-render`의 "패널 용량 최적화" 참조). 이미 검증·릴리스된 회차(ep01 등)에 소급 적용하지 않는다 — 게이트가 FAIL하는 것이 정상 동작이다.
...
RELEASE/ep{NN}/
  ...
  manifest.md5        # 검증 매니페스트 사본(게이트 통과 증빙)
...
- 조립 뷰어: `06_assembly/ep{NN}/index.html` (episode-compositor)
- 검수 보고: `06_assembly/ep{NN}/qa_report.md` (quality-reviewer)
- 연속성 기록: `06_assembly/continuity.md` (continuity-manager, 누적)
- 최종 패키지: `RELEASE/ep{NN}/` (showrunner)

입력 경로: 패널 `05_panels/ep{NN}/panel_*.png`, 레터링 `04_visual/ep{NN}_lettering.md`, 확정 대본 `03_episode/ep{NN}_script_final.md`, 반전 설계 `02_story/twist-plan.md`, 정본 `02_story/*`·`04_visual/*`.
