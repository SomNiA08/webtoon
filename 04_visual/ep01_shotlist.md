...
- subject: 카드
- emotion: —
- dialogue_ref: **카드 텍스트(2026-07-13 확정 — 사용자 제공, `00_input/brief.md` 참조)**: 기관명 `농심 이스포츠 아카데미`(1행, 큰 제목) + 개설 과정 3종 `리그 오브 레전드 · 발로란트 · FC 온라인`(1행, 제목보다 작게). 그 외(수강료·기간·강사진·실적 등)는 여전히 미확인 — **추가 정보 창작 금지**.
- fx/note: ⛔ **확정된 3항목(명칭·과정 3종) 외 커리큘럼·실적·수치 창작 0** · **panel_052와 시각적으로 명확히 분리** · 로고 이미지 사용 0(텍스트만)
- gap: **1.0×** / group: **B**
...
- subject: 카드
- emotion: —
- dialogue_ref: **카드 텍스트(2026-07-13 확정)**: 짧은 라벨 `문의 · 블로그 QR`(작게, 카드 상단 또는 QR 위) + **QR 코드 실물 이미지**(`00_input/academy_blog_qr.jpg`, 사용자 제공 원본을 codex 렌더 후 PIL로 픽셀 그대로 합성 — ⛔ **codex에게 QR을 그리게 하지 마라, 격자 정밀도가 깨져 스캔 불가능해진다**). 카드 중앙에 QR이 들어갈 **빈 정사각형 슬롯**만 codex가 렌더하고, 그 슬롯 좌표에 합성 스크립트가 실제 QR을 앉힌다.
- fx/note: ⛔ **연락처·모집 일정·취업률 창작 0** · ⛔ ***"신청하세요"* 류 독자 호명 0** · ⛔ **QR을 AI로 재생성 금지(원본 파일 합성만 허용)**
- gap: — (마지막 패널) / group: **C**
