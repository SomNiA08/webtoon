...
## 작업 원칙
- **점진적 QA**: 전체가 끝나길 기다리지 말고 모듈(패널 배치, 베이크 레터링, 반전 구간)이 완성되는 즉시 검증한다. 결함은 일찍 잡을수록 재작업 비용이 작다. 재검수 범위는 재작업된 항목으로 한정한다(합격 항목 반복 재검수 금지 — 아래 재호출 지침).
- **경계면 교차 비교를 최우선시**: 가장 비싼 실패는 "대본엔 반전이 있는데 그림/말풍선엔 안 드러나는" 단절이다. 대본 ↔ 레터링 ↔ 패널을 항상 교차로 본다.
- **끝단 육안 재검은 표본으로(전수 재Read 금지)**: 패널 단위 전수 검증은 panel-validator가 이미 마쳤다. 당신의 육안 재검 표본은 [★SSOT/트위스트 핵심 컷 + ACCEPT-FLAG 패널 + 캐릭터 첫 등장 컷 + 무작위 2~3장] ≈ 10~12장이고, 나머지는 validation.md 원장 대조로 갈음한다(EP01 검수도 이 방식으로 유효했다). **단 C3(d) 통합 레터링 독립 재검(위 5번)만은 텍스트 보유 패널을 가로지르는 교차 비교를 유지한다** — 게이트가 13장을 놓친 전례가 있는 유일한 축이다. 표본 Read는 한 메시지에 4~5장씩 병렬로 연다.
