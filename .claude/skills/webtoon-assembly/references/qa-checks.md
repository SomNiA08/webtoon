...
`{NN}`은 2자리 회차 번호로 치환한다. 기준 루트는 저장소 루트(산출물 디렉토리 00_input~RELEASE가 루트에 있다).
...
```bash
EP=01; H="06_assembly/ep${EP}/index.html"
# PANELS 배열이 참조하는 패널 개수
grep -o 'panel_[0-9]\{3\}\.png' "$H" | sort -u | wc -l
```
참조 수와 실제 PNG 수가 일치하는지 대조한다.

## 6. md5 중복 검사 (서로 다른 패널이 같은 이미지 — EP01 실제 발생)
```bash
EP=01; DIR="05_panels/ep${EP}"
md5sum "$DIR"/panel_*.png | awk '{print $1}' | sort | uniq -d   # macOS는 md5 -r
```
출력이 비면 정상. 중복이 나오면 중복 패널을 삭제 후 단독 재렌더 요청(크기/헤더 검사로는 못 잡는다).

## 7. 매니페스트 정합 (무검증 재렌더 색출 — EP01 실제 발생)
```bash
EP=01
# 매니페스트는 (cd 05_panels/ep{NN} && md5sum *.png > ../../04_visual/ep{NN}_manifest.md5)로 생성됨(파일명만 기록)
(cd "05_panels/ep${EP}" && md5sum -c "../../04_visual/ep${EP}_manifest.md5")
```
FAILED가 하나라도 나오면 검증 시점 이후 파일이 바뀐 것 → 해당 패널을 panel-validator 재검증으로 되돌리고 리포트·매니페스트를 갱신한다. 매니페스트가 없으면 panel-validator에 생성을 요청한다.

## 원샷 게이트 (1~4·6·7 + RELEASE 사본 대조 일괄)
```bash
bash _verify_release.sh 01   # exit 0 = 사인오프 가능, 그 외 = 사인오프 거부
```
