...
- 전체 렌더 완료를 기다리지 않는다. PNG가 생기는 대로(완료분부터) 검증을 시작해 후반 렌더와 앞 패널 검증을 겹친다.
